...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' B.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" B.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' B.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" B.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' B.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" B.podspec
...
VersionLineNumber=`grep -nE 's.version.*=' B.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" B.podspec
...
pod repo push ${Repo} B.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
